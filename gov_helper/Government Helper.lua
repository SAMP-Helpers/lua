...
print('[Government Helper] Скрипт инициализирован. Версия: ' .. thisScript().version)
-------------------------------------------- JSON SETTINGS ---------------------------------------------
local configDirectory = getWorkingDirectory():gsub('\\','/') .. "/GOV Helper"
local path_helper = getWorkingDirectory():gsub('\\','/') .. "/GOV Helper.lua"
			...
			if #contents == 0 then
				...
				if result then
					...
					if settings.general.version ~= thisScript().version then
						print('[Government Helper] Новая версия, сброс настроек!')
						settings = default_settings
						save_settings()
						reload_script = true
					else
...
function load_notes()
	if doesFileExist(path_notes) then
        ...
        if file then
            ...
            file:close()
			if #contents == 0 then
				print('[Government Helper] Не удалось открыть файл с заметками!')
				print('[Government Helper] Причина: этот файл пустой')
			else
				local result, loaded = pcall(decodeJson, contents)
				if result then
					notes = loaded
					print('[Government Helper] Заметки инициализированы!')
				else
					print('[Government Helper] Не удалось открыть файл с заметками!')
					print('[Government Helper] Причина: Не удалось декодировать json (ошибка в файле)')
				end
			end
        else
			print('[Government Helper] Не удалось открыть файл с заметками!')
			print('[Government Helper] Причина: ')
        end
	else
		print('[Government Helper] Не удалось открыть файл с заметками!')
		print('[Government Helper] Причина: этого файла нету в папке '..configDirectory)
	end
end
function save_notes()
    ...
    if file then
        ...
        file:write(result and encoded or "")
        file:close()
		print('[Government Helper] Заметки сохранены!')
        return result
    else
        print('[Government Helper] Не удалось сохранить заметки, ошибка: ', errstr)
        return false
    end
end
load_notes()
...
local commands = {
	commands = {
		{ cmd = 'z' , description = 'Привествие игрока' , text = 'Здравствуйте, меня зовут {my_ru_nick}, чем я могу помочь?', arg = '{arg_id}' , enable = true , waiting = '1.200', deleted = false },
		...
		{ cmd = 'visa' , description = 'Выдать рабочую визу для VC' ,  text = 'Стоимость услуги составляет 600 тысяч. Вы согласны?&Если да, то приступаем к оформлению&{pause}&/do Бланк для оформления визы находится в кармане.&/me засунув руку в карман, взял{sex} бланк, после чего протянул{sex} его человеку напротив&/todo Впишите сюда Ваши данные и поставьте подпись снизу*протягивая лист с ручкой&/givevisa {arg_id}', arg = '{arg_id}' , enable = true, waiting = '1.200', deleted = false   },
		{ cmd = 'car' , description = 'Превратить личный т/c в сертификат' , text = 'Перед тем, как начать, попрошу полностью опустошить багажник и снять весь тюнинг&А также убедиться, что пробег меньше либо равен 200 км&Если Вы все сделали, то можем приступать&{pause}&Окей, приступаем&/do Бланк для получения сертификата находится под в кармане.&/me засунув руку в карман, взял{sex} бланк, после чего протянул{sex} его человеку напротив&/todo Впишите сюда Ваши данные и поставьте подпись снизу*протягивая лист с ручкой&/givepass {arg_id}', arg = '{arg_id}' , enable = true, waiting = '1.200', deleted = false   },
		{ cmd = 'pass' , description = 'Исправить дату рождения в паспорте' ,  text = '/do Бланк для замены информации в паспорте находится в кармане.&/me засунув руку в карман, взял{sex} бланк, после чего протянул{sex} его человеку напротив&/todo Впишите сюда новую дату и поставьте подпись снизу*протягивая лист с ручкой&/givepass {arg_id}' , arg = '{arg_id}' , enable = true , waiting = '1.500', deleted = false  },		
		{ cmd = 'visit' , description = 'Показать визитку адвоката' ,  text = '/me вытащил{sex} из нагрудного кармана визитку адвоката&/do На визитке написано: "{my_ru_nick}, адвокат штата".&/showvisit {arg_id}' , arg = '{arg_id}', enable = true, waiting = '1.200', deleted = false} , 
		...
		{ cmd = 'exp' , description = 'Выгнать игрока из правительства' ,  text = 'Извините, но я вынужден{sex} выгнать вас из здания за неподобающее поведение.&/me схватил{sex} человека за руку и повел{sex} к выходу&/me открыв дверь рукой, вывел человека на улицу&/expel {arg_id} Неадекватное поведение' , arg = '{arg_id}' , enable = true , waiting = '1.200' , deleted = false },
		...
		{ cmd = 'frisk', description = 'Обыск +7 ранг', text = '/do Перчатки на поясе.&/me схватил перчатки и одел&/do Перчатки одеты.&/me начал нащупывать человека напротив&/frisk {arg_id}', arg = '{arg_id}', enable = false, waiting = '2.500' }
	},
	commands_manage = {
...
function load_commands()
	if doesFileExist(path_commands) then
        ...
        if file then
            ...
            file:close()
			if #contents == 0 then
				print('[Government Helper] Не удалось открыть файл с командами!')
				print('[Government Helper] Причина: этот файл пустой')
			else
				local result, loaded = pcall(decodeJson, contents)
				if result then
					if tostring(settings.general.version) ~= tostring(thisScript().version) then 
						print('[Government Helper] Обнаружена иная версия, пробую добавить новые команды')
						...
						for category, _ in pairs(commands) do
							...
							for key, value in pairs(commands[category]) do
								if temp_commands[category][key] == nil then
									...
									print('[Government Helper] Добавляю команду /' .. value.cmd)
								end
							end
						end
						save_commands()
						...
						for _, command in pairs(loaded.commands) do
							if not command.bind then
								print('[Government Helper] Обновляю команду /' .. command.cmd .. ' (поддержка биндов)')
								command.bind = "{}"
							end
						end
					...
					print('[Government Helper] Все команды инициализирован!')
			...
			print('[Government Helper] Не удалось открыть файл с командами!')
			print('[Government Helper] Причина: ', errstr)
        end
	else
		print('[Government Helper] Не удалось открыть файл с командами!')
		print('[Government Helper] Причина: этого файла нету в папке '..configDirectory)
		print('[Government Helper] Инициализация стандартных команд...')
		save_commands()
		load_commands()
	end
end
function save_commands()
    ...
    if file then
        ...
        file:write(result and encoded or "")
        file:close()
		print('[Government Helper] Ваши команды сохранены!')
        return result
    else
        print('[Government Helper] Не удалось сохранить команды хелпера, ошибка: ', errstr)
        return false
    end
end
load_commands()
...
if not settings.general.autofind_dpi then
	print('[Government Helper] Применение авто-размера менюшек...')
	if isMonetLoader() then
	...
	print('[Government Helper] Установлено значение: ' .. settings.general.custom_dpi)
	save_settings()
end
---------------------------------------------- Mimgui -----------------------------------------------------
	...
	if hotkey_no_errors then
		...
		function updateHotkeyForCommand(command)
			...
			if command.arg == '' and command.bind ~= nil and command.bind ~= '{}' and command.bind ~= '[]' then
				...
				print('[Government Helper] Создан хоткей для команды /' .. command.cmd .. ' на клавишу ' .. getNameKeysFrom(command.bind))
				--sampAddChatMessage('[Government Helper] {ffffff}Создан хоткей для команды ' .. message_color_hex .. '/' .. command.cmd .. ' {ffffff}на клавишу '  .. message_color_hex .. getNameKeysFrom(command.bind), message_color)
			end
		end
...
function welcome_message()
	...
	print('[Government Helper] Загрузка хелпера прошла успешно!')
...
function find_and_use_command(cmd, cmd_arg)
	...
	if not check then
		sampAddChatMessage('[Government Helper] {ffffff}Ошибка, не могу найти бинд для выполнения этой команды!', message_color)
		play_error_sound()
		return
	end
end
...
function play_error_sound()
	...
	show_arz_notify('error', 'Government Helper', "Произошла ошибка!", 1500)
end
...
function ifCommandPause()
	if command_stop then 
		...
		sampAddChatMessage('[Government Helper] {ffffff}Отыгровка команды успешно остановлена!', message_color)  
		return true
	else
		return false
	end
	...
	if isParamSampID(id) then 
		...
		if isMonetLoader() or settings.general.bind_fastmenu == nil then
			if not FastMenuPlayers[0] then
				sampAddChatMessage('[Government Helper] {ffffff}Используйте ' .. message_color_hex .. '/gm [ID]', message_color)
			end
		elseif settings.general.bind_fastmenu and settings.general.use_binds and hotkey_no_errors then
			sampAddChatMessage('[Government Helper] {ffffff}Используйте ' .. message_color_hex .. '/gm [ID] {ffffff}или наведитесь на игрока через ' .. message_color_hex .. 'ПКМ + ' .. getNameKeysFrom(settings.general.bind_fastmenu), message_color) 
		else
			sampAddChatMessage('[Government Helper] {ffffff}Используйте ' .. message_color_hex .. '/gm [ID]', message_color)
		end 
	end 
...
function sampev.onShowDialog(dialogid, style, title, button1, button2, text)
	...
	if members_check and title:find('(.+)%(В сети: (%d+)%)') then -- мемберс 
		...
		elseif #members_new ~= 0 then
			...
			sampSendDialogResponse(dialogid, 0, 0, 0)
			sampAddChatMessage('[Government Helper]{ffffff} Список сотрудников пуст!', message_color)
			members_check = false
        end
        return false
    end
...
imgui.OnFrame(
    ...
    function(player)
		...
		if imgui.BeginTabBar('пон') then	
			if imgui.BeginTabItem(fa.ID_CARD..u8' Главное меню') then
				...
				if imgui.BeginChild('##3', imgui.ImVec2(589 * settings.general.custom_dpi, 97 * settings.general.custom_dpi), true) then
					...
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText(u8"RP отыгровка оружий")
					...
					if settings.general.auto_uval then
						...
						if imgui.CenterColumnSmallButton(u8'Включить##auto_uval') then
							if tonumber(settings.player_info.fraction_rank_number) == 9 or tonumber(settings.player_info.fraction_rank_number) == 10 then 
								...
								settings.general.auto_uval = false
								sampAddChatMessage('[Government Helper] {ffffff}Эта Функция доступна только лидеру и заместителям!',message_color)
							end
						end
					end
					imgui.Columns(1)
					...
					if imgui.BeginTabItem(fa.BARS..u8' Дополнительные функции') then 
						if imgui.BeginChild('##999', imgui.ImVec2(589 * settings.general.custom_dpi, 333 * settings.general.custom_dpi), true) then
							if isMonetLoader() then
								...
								if imgui.RadioButtonIntPtr(u8" Только используя команду /gm [ID игрока]", fastmenu_type, 0) then
									fastmenu_type[0] = 0
									settings.general.mobile_fastmenu_button = false
									save_settings()
									FastMenuButton[0] = false
								end
								if imgui.RadioButtonIntPtr(u8' Используя команду /gm [ID игрока] или кнопку "Взаимодействие" в левом углу экрана', fastmenu_type, 1) then
									fastmenu_type[0] = 1
								...
								if hotkey_no_errors then
									if settings.general.use_binds then
										...
										if settings.general.use_binds and hotkey_no_errors then
											...
											imgui.Separator()
											imgui.CenterText(u8'Открытие быстрого меню взаимодействия с игроком (аналог /gm):')
			...
			if imgui.BeginTabItem(fa.GEAR..u8' Настройки скрипта') then 
				...
				imgui.SameLine()
				if imgui.SmallButton('FD Helper (Author MTG Mods)') then
				...
				imgui.SameLine()
				if imgui.SmallButton('Government Helper') then
				...
				if settings.general.custom_dpi == slider_dpi[0] then
					...
					if imgui.SmallButton(fa.CIRCLE_ARROW_RIGHT .. u8' Применить и сохранить') then
						...
						save_settings()
						sampAddChatMessage('[Government Helper] {ffffff}Перезагрузка скрипта для пременения размера окон...', message_color)
						reload_script = true
						thisScript():reload()
					end
				end
				...
				if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. u8' Предупреждение ##off', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar ) then
					...
					if imgui.Button(fa.POWER_OFF .. u8' Да, выгрузить', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
						...
						if not isMonetLoader() then 
							sampAddChatMessage('[Government Helper] {ffffff}Либо используйте ' .. message_color_hex .. 'CTRL {ffffff}+ ' .. message_color_hex .. 'R {ffffff}чтобы запустить хелпер.', message_color)
						end
						thisScript():unload()
					end
					imgui.End()
...
imgui.OnFrame(
    ...
    function(player)
		...
		if imgui.Button(fa.KEYBOARD .. u8' Бинд (для ПК)', imgui.ImVec2(imgui.GetMiddleButtonX(5), 0)) then
			if isMonetLoader() then
				sampAddChatMessage('[Government Helper] {ffffff}Данная функция доступа только на ПК!', message_color)
			else
				if hotkey_no_errors then
					if ComboTags[0] == 0 then
						if settings.general.use_binds then 
							...
							sampAddChatMessage('[Government Helper] {ffffff}Сначало включите рабоспособность хоткеев в Команды и отыгровки - Доп. функии', message_color)
						end
					else
						sampAddChatMessage('[Government Helper] {ffffff}Данная функция доступа только если команда "Без аргументов"', message_color)
					end
				else
					sampAddChatMessage('[Government Helper] {ffffff}Данная функция недоступа, причина отсуствуют файлы библиотеки mimgui_hotkeys!', message_color)
				end
			end
		end
		if imgui.BeginPopupModal(fa.KEYBOARD .. u8' Бинд для команды /' .. change_cmd, _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize) then
			...
			if hotkeyObject then
				...
				if hotkeyObject:ShowHotKey() then
					...
					sampAddChatMessage('[Government Helper] {ffffff}Создан хоткей для команды ' .. message_color_hex .. '/' .. change_cmd .. ' {ffffff}на клавишу '  .. message_color_hex .. getNameKeysFrom(change_bind), message_color)
				end
			else
				...
				--sampAddChatMessage('[Government Helper] {ffffff}Создан новый хоткей, необходимо пересоздать интерфейс. Откройте заново эту менюшку', message_color)
				--imgui.CloseCurrentPopup()
			end
			imgui.Separator()
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.BUILDING_SHIELD.." Government Helper##rank", GiveRankMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
		if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.BUILDING_SHIELD .. " Government Helper##CommandStopWindow", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize )
		if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY - 50 * settings.general.custom_dpi), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.BUILDING_SHIELD.." Government Helper##CommandPauseWindow", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize )
		if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
