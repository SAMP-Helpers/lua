...
---------------------------------------------- Mimgui -----------------------------------------------------
--[[
		imgui.Begin("название")  					-- Создаём новое окно с заголовком 'название'
        imgui.Text("текст")         				-- Вывести определённый текст
        imgui.End()                 				-- Объявляем конец данного окна
		imgui.Button()								-- Создание кнопки
		imgui.InputText()							-- Создание поля для ввода текста
		imgui.TextDisabled()						-- Отключённый текст
		imgui.SetCursorPosX()						-- Позволяет задать положение функции по горизонтали
		imgui.SetCursorPosY()						-- Позволяет задать положение функции по вертикали
		imgui.SetCursorPos(imgui.ImVec2(50, 170)) 	-- 50 = по горизонтали, 170 по вертикали
		imgui.BeginTooltip()						-- Вывод определённого текста при наведении(подсказки)
		imgui.Columns(количество)					-- Количество столбцов в таблице
		imgui.CenterText(текст)						-- Вывод текста по середине
		imgui.EndTooltip()							-- Завершение отображение подсказки
		imgui.Separator()							-- Визуальный разделитель элементов на экране
		imgui.ColoredButton(u8'название кнопки', 'цвет в HEX формате', прозрачность,imgui.ImVec2(размер кнопки(X,Y)))
...
local item_list = { u8 'Без аргумента', u8 '{arg} - принимает любой аргумент', u8 '{arg_id} - принимает только аргумент ID игрока',
	u8 '{arg_id} {arg2} - принимает 2 аругмента: ID игрока и любой аргумент', u8 '{arg_id} {arg2} {arg3} - принимает 3 аргумента: ID игрока, одну цифру, и любой аргумент',
	u8 '{arg_id} {arg2} {arg3} {arg4} - принимает 4 аргумента: ID игрока, одну цифру и два любых аргумента' }
...
local tagReplacements = {
	...
	get_storecar_model = function()
		...
		if closest_car then
			...
			sampAddChatMessage("[Prison Helper] {ffffff}Не удалось получить модель ближайшего транспорта с водителем!",
				0x6E6E6E)
			return ''
		end
	end,
...
function initialize_commands()
	-- Регистрация команд из commands.commands
	registerCommandsFrom(commands.commands)
	-- Регистрация команд из commands.commands_manage
	...
	if (settings.general.auto_doklad) then
		...
		if text:find('%[Патрулирование%] {ffffff}Доложите о {ff6666}начале выполнения маршрута в рацию %(/r%){ffffff}, чтобы продолжить.') then
			...
			sampSendChat('/r Докладывает ' ..
				...
				': ' ..
				settings.player_info.name_surname .. '. Пост: ' .. postStr ..
				'. Состояние: ' .. settings.general.post_status .. '')
		end
	end
...
function sampev.onShowDialog(dialogid, style, title, button1, button2, text)
	...
	if vc_vize_bool and title:find('Выдача разрешений на поездки Vice City') then -- VS Visa [1]
		vc_vize_bool = false
		sampSendChat("/r Сотруднику " ..
			TranslateNick(sampGetPlayerNickname(tonumber(vc_vize_player_id))) .. " выдана виза Vice City!")
	...
	if vc_vize_bool and title:find('Забрать разрешение на поездки Vice City') then -- VS Visa [2]
		vc_vize_bool = false
		sampSendChat("/r У сотрудника " ..
			TranslateNick(sampGetPlayerNickname(tonumber(vc_vize_player_id))) .. " была изьята виза Vice City!")
		...
		if imgui.BeginTabBar('пон') then
				...
				if imgui.BeginTabBar('Tabs2') then
						...
						if tonumber(settings.player_info.fraction_rank_number) >= 5 or tonumber(settings.player_info.fraction_rank_number) == 9 or tonumber(settings.player_info.fraction_rank_number) == 10 then
							if imgui.BeginChild('##1', imgui.ImVec2(690 * MONET_DPI_SCALE, 312 * MONET_DPI_SCALE), true) then -- Размеры вкладки "Команды для 5+"
								...
								for index, command in ipairs(commands.commands_senior_staff) do
									...
									imgui.SameLine()
									if imgui.SmallButton(fa.TRASH_CAN .. '##' .. command.cmd) then
										imgui.OpenPopup(fa.TRIANGLE_EXCLAMATION .. u8 ' Предупреждение ##' .. command
											.cmd)
									end
									if imgui.IsItemHovered() then
										imgui.SetTooltip(u8 "Удаление команды /" .. command.cmd)
							...
							imgui.CenterText(u8 "Необходимо иметь ранг выше 5, у вас же " ..
								settings.player_info.fraction_rank_number .. u8 " ранг!")
							imgui.Separator()
						end
						imgui.EndTabItem()
					end
					if imgui.BeginTabItem(fa.BARS .. u8 ' Команды для 9-10 рангов') then
						if tonumber(settings.player_info.fraction_rank_number) == 9 or tonumber(settings.player_info.fraction_rank_number) == 10 then
								...
								for index, command in ipairs(commands.commands_manage) do
									...
									if imgui.SmallButton(fa.TRASH_CAN .. '##' .. command.cmd) then
										imgui.OpenPopup(fa.TRIANGLE_EXCLAMATION ..
											u8 ' Предупреждение ##9-10' .. command.cmd)
									end
									if imgui.IsItemHovered() then
										imgui.SetTooltip(u8 "Удаление команды /" .. command.cmd)
									end
							...
							imgui.Separator()
							imgui.CenterText(u8 "У вас нет доступа к данным командам!")
							imgui.CenterText(u8 "Необходимо иметь 9 или 10 ранг, у вас же " ..
								settings.player_info.fraction_rank_number .. u8 " ранг!")
							imgui.Separator()
						end
						imgui.EndTabItem()
						...
						if imgui.BeginChild('##99', imgui.ImVec2(689 * MONET_DPI_SCALE, 342 * MONET_DPI_SCALE), true) then
							if isMonetLoader() then
								...
								if hotkey_no_errors then
									...
									imgui.CenterText(fa.TRIANGLE_EXCLAMATION ..
										u8 ' Ошибка: отсуствуют файлы библиотеки!')
								end
								imgui.Separator()
			...
			if imgui.BeginTabItem(fa.PLAY .. u8 ' Функции') then
				if imgui.BeginChild('##1', imgui.ImVec2(689 * MONET_DPI_SCALE, 195 * MONET_DPI_SCALE), true) then -- Размеры первой вкладки
					...
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText(u8 "Информация о количестве работы")
					imgui.SameLine(nil, 5)
					imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
					...
					imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8 "Состояние поста в автодоклад(/r).")
					end
					imgui.SetColumnWidth(-1, 280 * MONET_DPI_SCALE) -- Длина первого столбца
					imgui.NextColumn()
					if checkbox_accent_enable[0] then
					...
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText(u8 "Количество созданных патронов:")
					imgui.SameLine(nil, 5)
					imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
					...
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText(u8 "Количество поставок в МЮ:")
					imgui.SameLine(nil, 5)
					imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
					...
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText(u8 "Количество поставок ингредиентов:")
					imgui.SameLine(nil, 5)
					imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
...
imgui.OnFrame(
	...
	function(player)
		...
		imgui.Separator()
		imgui.CenterText(u8 'Предпросмотр: /d ' ..
					...
					lua_thread.create(function()
						...
						for _, line in ipairs(lines) do
							...
							if wait_tag then
								...
								if debug_mode then
									sampAddChatMessage(
										'[Prison Helper DEBUG] Отправляю сообщение: ' .. line, message_color)
								end
				...
				if imgui.Button(fa.PASSPORT .. u8 " Попросить документы", imgui.ImVec2(-1, 25 * MONET_DPI_SCALE)) then
					lua_thread.create(function()
						...
						sampSendChat("/n " ..
							...
							", используйте /showpass [ID] , /showmc [ID] , /showlic [ID]")
						wait(2000)
			...
			if imgui.BeginChild('sobes3', imgui.ImVec2(150 * MONET_DPI_SCALE, -1), true) then
				...
				if imgui.Selectable(u8 "Нету жилья") then
					lua_thread.create(function()
						...
						wait(2000)
						sampSendChat(
							"У вас нету жилья. Арендуйте себе отель либо подселитесь в дом, затем приходите к нам!")
					end)
				end
						...
						if chapter.item then
							for _, item in ipairs(chapter.item) do
								local popup_id = fa.TRIANGLE_EXCLAMATION ..
									u8 ' Перепроверьте данные перед выдачей повышения##' ..
								...
								if imgui.BeginPopupModal(popup_id, nil, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize) then
									...
									if imgui.Button(fa.STAR .. u8 ' Выдать повышение срока', imgui.ImVec2(200 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
										...
										if settings.general.use_form_su then
											find_and_use_command('Прошу обьявить в розыск %{arg2%} степени дело N',
					...
					if chapter.name:rupper():find(input_sum_decoded:rupper()) then
						if imgui.CollapsingHeader(u8(chapter.name)) then
							if chapter.item then
								for _, item in ipairs(chapter.item) do
									local popup_id = fa.TRIANGLE_EXCLAMATION ..
										u8 ' Перепроверьте данные перед выдачей розыска##' ..
										item.text .. item.lvl .. item.reason
									if imgui.Button(u8(item.text) .. '##' .. item.text .. item.lvl .. item.reason, imgui.ImVec2(imgui.GetMiddleButtonX(1), 25 * MONET_DPI_SCALE)) then
										imgui.OpenPopup(popup_id)
									end
										...
										if imgui.Button(fa.STAR .. u8 ' Выдать повышение срока', imgui.ImVec2(200 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
											...
											if settings.general.use_form_su then
												find_and_use_command(
													'Прошу обьявить в розыск %{arg2%} степени дело N%(%{arg_id%}%)%. Причина%: %{arg3%}',
													player_id .. ' ' .. item.lvl .. ' ' .. item.reason)
											else
												find_and_use_command('/punish {arg_id} {arg2} 2 {arg3}',
													player_id .. ' ' .. item.lvl .. ' ' .. item.reason)
