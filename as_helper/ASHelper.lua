...
local default_settings = {
	...
	commands = {
		...
		{ cmd = 'zd' , description = 'Привествие игрока' , text = 'Здраствуйте, я сотрудник данного центра лицензирования - {my_ru_nick}, чем я могу Вам помочь?&/do На груди висит бейджик с надписью {fraction_rank} {fraction_tag}.', arg = '{arg_id}' , enable = true , waiting = '1.200', deleted = false },
		...
		{ cmd = 'expel' , description = 'Выгнать игрока из ЦЛ' ,  text = 'Вы больше не можете здесь находиться, я выгоняю вас из автошколы!&/me схватив человека ведёт к выходу из автошколы и закрывает за ним дверь&/expel {arg_id} {expel_reason}' , arg = '{arg_id}' , enable = true , waiting = '1.200' , deleted = false },
		{ cmd = 'price' , description = 'Ознакомить игрока со списком цен' , text = '/do На столе лежит список цен лицензий.&/me подвинул лист поближе к гражданину&Вот, ознакомьтесь.&/do На листке написано...&/do Водительские права - {price_avto}&/do Права на мото транспорт - {price_moto}&/do Лицензия на рыбалку - {price_fish}&/do Лицензия на водный транспорт - {price_swim}&/do Лицензия на оружие - {price_gun}&/do Лицензия на охоту - {price_hunt}&/do Лицензия на раскопки - {price_klad}&/do Лицензия на полёты - {price_fly}&/do Лицензия на полёты продаётся в Авиашколе Лас-Вентурас. ' , arg = '' , enable = true , waiting = '1.200' , deleted = false},
		{ cmd = 'givelicense' , description = 'Выписать лицензию игроку' , text = '/todo Хорошо, минуточку *повернув КПК в свою сторону и что-то выбирая на нем&/do На КПК выбрана нужная лицензия, и показана соответствующая цена.&/me подтверждает выбор лицензии&/do КПК печатает чек.&/do Чек готов.&/do На чеке написана сумма оплаты.&/me передал чек человеку напротив&/me ввел данные гражданина в КПК&/do Данные введены верно.&/givelicense {arg_id}&{pause}&Вот ваша лицензия, всего Вам хорошего!' , arg = '{arg_id}' , enable = true , waiting = '1.200' , deleted = false},
		{ cmd = 'givelic' , description = 'Аналог /givelicense(укороченная версия)' , text = '/me взял со стола бланк заполнил ручкой бланк на получение лицензии&/do Спустя некоторое время бланк на получение лицензии был заполнен.&/me распечатав лицензию передал её человеку напротив&/givelicense {arg_id}' , arg = '{arg_id}' , enable = true , waiting = '1.200' , deleted = false},
		{ cmd = 'medka' , description = 'Запросить медкарту для выдачи лицензии' , text = 'Чтобы получить эту лицензию, покажите мне вашу мед.карту&/n {get_nick({arg_id})} введите /showmc {my_id}&{pause}&/me взял со стола бланк заполнил ручкой бланк на получение лицензии&/do Спустя некоторое время бланк на получение лицензии был заполнен.&/me распечатав лицензию передал её человеку напротив&/givelicense {arg_id}&{pause}&Вот ваша лицензия, всего Вам хорошего!' , arg = '{arg_id}' , enable = true , waiting  = '1.200', deleted = false },
	},
	commands_manage = {
		{ cmd = 'book' , description = 'Выдача игроку трудовой книги' , text = 'Оказывается у вас нету трудовой книги, но не переживайте!&Сейчас я вам выдам её, вам не нужно никуда ехать, секунду...&/me достаёт из своего кармана новую трудовую книжку и ставит на ней печать {fraction_tag}&/todo Берите*передавая трудовую книгу человеку напротив&/givewbook {arg_id} 100&/n {get_nick({arg_id})}, примите предложение в /offer чтобы получить трудовую книгу!' , arg = '{arg_id}', enable = true, waiting = '1.200', deleted = false  },
		{ cmd = 'inv' , description = 'Принятие игрока в фракцию' , text = '/do В кармане жилетки есть связка с ключами от раздевалки.&/me достаёт из жилетки один ключ из связки ключей от раздевалки&/todo Возьмите, это ключ от нашей раздевалки*передавая ключ человеку напротив&/invite {arg_id}&/r {get_ru_nick({arg_id})} наш новый сотрудник!' , arg = '{arg_id}', enable = true, waiting = '1.200' , deleted = false  },
		...
		{ cmd = 'fmutes' , description = 'Выдать мут сотруднику (10 min)' , text = '/fmutes {arg_id} {expel_reason}' , arg = '{arg_id}', enable = true, waiting = '1.200' , deleted = false  },
		{ cmd = 'funmute' , description = 'Снять мут сотруднику' , text = '/funmute {arg_id}&' , arg = '{arg_id}', enable = true, waiting = '1.200' , deleted = false  },
		{ cmd = 'vig' , description = 'Выдача выговора cотруднику' , text = '/me достаёт из кармана свой телефон и заходит в базу данных {fraction_tag}&/me изменяет информацию о сотруднике {get_ru_nick({arg_id})} в базе данных {fraction_tag}&/me выходит с базы данных и убирает телефон обратно в карман&/fwarn {arg_id} {arg2}' , arg = '{arg_id} {arg2}', enable = true, waiting = '1.200'  , deleted = false },
		{ cmd = 'unvig' , description = 'Снятие выговора cотруднику' , text = '/me достаёт из кармана свой телефон и заходит в базу данных {fraction_tag}&/me изменяет информацию о сотруднике {get_ru_nick({arg_id})} в базе данных {fraction_tag}&/me выходит с базы данных и убирает телефон обратно в карман&/unfwarn {arg_id}' , arg = '{arg_id}', enable = true, waiting = '1.200' , deleted = false  },
		{ cmd = 'unv' , description = 'Увольнение игрока из фракции' , text = '/me достаёт из кармана свой телефон и заходит в базу данных {fraction_tag}&/me изменяет информацию о сотруднике {get_ru_nick({arg_id})} в базе данных {fraction_tag}&/me выходит с базы данных и убирает свой телефон обратно в карман&/uninvite {arg_id} {arg2}' , arg = '{arg_id} {arg2}', enable = true, waiting = '1.200' , deleted = false  },
...
{price_avto} - Цена выдачи лицензии на авто
{price_moto} - Цена выдачи лицензии на мото
{price_fly} - Цена выдачи лицензии на воздушный транспорт 
{price_swim} - Цена выдачи лицензии на водный транспорт
{price_klad} - Цена выдачи лицензии на раскопки
{price_hunt} - Цена выдачи лицензии на охоту
{price_taxi} - Цена выдачи лицензии для таксистов 
{price_mexa} - Цена выдачи лицензии для механика 
{price_fish} - Цена выдачи лицензии на рыболовство
{price_gun} - Цена выдачи лицензии на оружие
...
function welcome_message()
	...
	show_cef_notify('info', 'AS Helper', "Загрузка хелпера прошла успешно!", 3000)
	...
	if isParamSampID(id) then 
		...
		if isMonetLoader() or settings.general.bind_fastmenu == nil then
			if not FastMenuPlayers[0] then
				sampAddChatMessage('[AS Helper] {ffffff}Используйте ' .. message_color_hex .. '/shm [ID]', message_color)
			end
		elseif settings.general.bind_fastmenu and settings.general.use_binds and hotkey_no_errors then
			sampAddChatMessage('[AS Helper] {ffffff}Используйте ' .. message_color_hex .. '/shm [ID] {ffffff}или наведитесь на игрока через ' .. message_color_hex .. 'ПКМ + ' .. getNameKeysFrom(settings.general.bind_fastmenu), message_color) 
		else
			sampAddChatMessage('[AS Helper] {ffffff}Используйте ' .. message_color_hex .. '/shm [ID]', message_color)
		end 
	end 
...
function show_leader_fast_menu(id)
	if isParamSampID(id) then
		...
		if isMonetLoader() or settings.general.bind_leader_fastmenu == nil then
			sampAddChatMessage('[AS Helper] {ffffff}Используйте ' .. message_color_hex .. '/shlm [ID]', message_color)
		elseif settings.general.bind_leader_fastmenu and settings.general.use_binds and hotkey_no_errors then
			sampAddChatMessage('[AS Helper] {ffffff}Используйте ' .. message_color_hex .. '/shlm [ID] {ffffff}или наведитесь на игрока через ' .. message_color_hex .. 'ПКМ + ' .. getNameKeysFrom(settings.general.bind_leader_fastmenu), message_color) 
		else
			sampAddChatMessage('[AS Helper] {ffffff}Используйте ' .. message_color_hex .. '/shlm [ID]', message_color)
		end 
	end
	...
	if title:find('Основная статистика') and check_stats then -- получение статистики
		...
		if text:find("{FFFFFF}Организация: {B83434}%[(.-)]") then
			...
			if settings.player_info.fraction == 'Не имеется' then
				...
				if settings.player_info.fraction == 'Центр лицензирования' or settings.player_info.fraction == 'Центр лицензирования SF' then
					settings.player_info.fraction_tag = 'ЦЛ'
				else
		...
		if id == 220 then
			...
			if raknetBitStreamReadInt8(bs) == 17 then
				...
				if cmd2:find('Основная статистика') and check_stats then -- /hme
					sampAddChatMessage('[AS Helper] {ffffff}Ошибка, не могу получить данные из нового CEF диалога!', message_color)
					sampAddChatMessage('[AS Helper] {ffffff}Включите старый (класичесский) вид диалогов в /settings - Кастомизация интерфейса', message_color)
    ...
    function(player)
		...
		if imgui.BeginTabBar('Tabs') then	
			...
			if imgui.BeginTabItem(fa.MONEY_CHECK_DOLLAR..u8' Ценовая политика') then 
				...
				if imgui.InputText(u8'  Выдача лицензии на авто ', input_avto, 8) then
					settings.price.avto = u8:decode(ffi.string(input_avto))
					save_settings()
				end
				imgui.Separator()
				imgui.PushItemWidth(70 * MONET_DPI_SCALE)
				if imgui.InputText(u8'  Выдача лицензии на мото ', input_moto, 8) then
					settings.price.moto = u8:decode(ffi.string(input_moto))
					save_settings()
				end
				imgui.Separator()
				imgui.PushItemWidth(70 * MONET_DPI_SCALE)
				if imgui.InputText(u8'  Выдача лицензии на полеты ', input_fly, 8) then
					settings.price.flys = u8:decode(ffi.string(input_fly))
					save_settings()
				end
				imgui.Separator()
				imgui.PushItemWidth(70 * MONET_DPI_SCALE)
				if imgui.InputText(u8'  Выдача лицензии на плавание ', input_swim, 8) then
					settings.price.swim = u8:decode(ffi.string(input_swim))
					save_settings()
				end
				imgui.Separator()
				imgui.PushItemWidth(70 * MONET_DPI_SCALE)
				if imgui.InputText(u8'  Выдача лицензии на раскопки ', input_klad, 8) then
					settings.price.klad = u8:decode(ffi.string(input_klad))
					save_settings()
				end
				imgui.Separator()
				imgui.PushItemWidth(70 * MONET_DPI_SCALE)
				if imgui.InputText(u8'  Выдача лицензии на охоту', input_hunt, 8) then
					settings.price.hunt = u8:decode(ffi.string(input_hunt))
					save_settings()
				end
				imgui.Separator()
				imgui.PushItemWidth(70 * MONET_DPI_SCALE)
				if imgui.InputText(u8'  Выдача лицензии для таксистов ', input_taxi, 8) then
					settings.price.taxi = u8:decode(ffi.string(input_taxi))
					save_settings()
				end
				imgui.Separator()
				imgui.PushItemWidth(70 * MONET_DPI_SCALE)
				if imgui.InputText(u8'  Выдача лицензии для механиков ', input_mexa, 8) then
					settings.price.mexa = u8:decode(ffi.string(input_mexa))
					save_settings()
				end
				imgui.Separator()
				imgui.PushItemWidth(70 * MONET_DPI_SCALE)
				if imgui.InputText(u8'  Выдача лицензии на рыболовство ', input_fish, 8) then
					settings.price.fish = u8:decode(ffi.string(input_fish))
					save_settings()
				end
				imgui.Separator()
				imgui.PushItemWidth(70 * MONET_DPI_SCALE)
				if imgui.InputText(u8'  Выдача лицензии на оружие ', input_gun, 8) then
			...
			if imgui.BeginTabItem(fa.GEAR..u8' Настройки') then 
				...
				imgui.Separator()
				imgui.Text(fa.CIRCLE_USER..u8" Разработчик данного хелпера: Fitalov & MTG MODS")
				imgui.Separator()
...
imgui.OnFrame(
    ...
    function(player)
		if player_id ~= nil and isParamSampID(player_id) then
			...
			if imgui.Button(fa.HOSPITAL_USER .. u8" Пригласить в организацию", imgui.ImVec2(-1, 25 * MONET_DPI_SCALE)) then
